...
prop_longerPolicyBetterOrEqual t x =
  ...
  forAll genValidInt $ \n ->
    ...
    in not (null ps) ==>
      let v1 = val ghgcase t ps x
          v2 = val ghgcase t (ps ++ [last ps]) x
      in v2 >= v1
...
prop_bestExtWorstCorrectLength =
  ...
  forAll genValidInt $ \n ->
    let ps = bi ghgcase t n
        allStates = GHGCase.getStates
    in length (bestExt ghgcase t ps) == length allStates
...
prop_bestExtGEQworst s =
  ...
  forAll genValidInt $ \t ->
    let ps = bi ghgcase t n
        bestPol = bestExt ghgcase t ps
        worstPol = worstExt ghgcase t ps
        bestVal = val ghgcase t (bestPol : ps) s
        worstVal = val ghgcase t (worstPol : ps) s
    in bestVal >= worstVal
...
prop_optExtDeterministic =
  ...
  forAll genValidInt $ \n ->
    let ps = bi ghgcase t n
        p1 = bestExt ghgcase t ps
        p2 = bestExt ghgcase t ps
    in p1 == p2
...
prop_bestExtPolicyIdempotent s =
  ...
  forAll genValidInt $ \n ->
    let ps = bi ghgcase t n
        bestP = bestExt ghgcase t ps
        newPs = bestP : ps
        v1 = val ghgcase t newPs s
        v2 = val ghgcase t newPs s
    in v1 == v2
...
prop_bestActionIsValid s =
  forAll genValidInt $ \t ->
  forAll genValidInt $ \n ->
    let (a, _) = best ghgcase t n s
        validActions = GHGCase.actions t s
...
prop_bestValMatchesVal s =
  forAll genValidInt $ \t ->
  forAll genValidInt $ \n ->
    let (a, v) = best ghgcase t n s
        ps = bi ghgcase (t + 1) (n - 1)
        p = Map.singleton s a
        v' = val ghgcase t (p : ps) s
    in v === v'
...
prop_bestValIsOptimal s =
  forAll genValidInt $ \t ->
  forAll genValidInt $ \n ->
    let (a, vBest) = best ghgcase t n s
        ps = bi ghgcase (t + 1) (n - 1)
        otherActions = GHGCase.actions t s
        altVals = [val ghgcase t (Map.singleton s a' : ps) s | a' <- otherActions]
...
prop_sumEqualToOne x = 
  ...
  forAll (genValidAction t x) $ \s ->
    sum (Prob.weights (GHGCase.next t x s)) >= 0.99 &&
    sum (Prob.weights (GHGCase.next t x s)) <= 1.01
...
prop_nextDeterministic x = 
  forAll genValidInt $ \t ->
  forAll (genValidAction t x) $ \s ->
    let d0 = unzip $ unProb (GHGCase.next t x s)
        d1 = unzip $ unProb (GHGCase.next t x s)
