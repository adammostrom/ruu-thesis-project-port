...
-- Manually handling the transitions
step :: (x -> [(x, Probability)]) -> x -> [(x, Probability)]
step f x = normalize (f x)

-- Normalize distribution (sum of probs <= 1.0)
...
-- Backwards induction
bi ::(TheoryNoMonad x y, Show x, Eq x, Ord x) =>  Int -> Int -> PolicySeq x y
bi _ 0 = []
bi t n =
...
val :: (TheoryNoMonad x y, Ord x, Show x) => Int -> PolicySeq x y -> x -> Val
val _ [] _ = 0
val t (p : ps) x =
  case Map.lookup x p of
    Nothing -> error "No action"
    Just y ->
      let nextStates = normalize (unProb $ next t x y)
      in sum [pr * (reward t x y x' + val (t + 1) ps x') | (x', pr) <- nextStates]



best :: (TheoryNoMonad x y, Show x, Eq x, Ord x) => Int -> Int -> x -> (y, Val)
best t n x
...
bestExt :: (TheoryNoMonad x y, Show x, Eq x, Ord x) =>  Int -> PolicySeq x y -> Policy x y
...
worstExt :: (TheoryNoMonad x y, Show x, Eq x, Ord x) =>  Int -> PolicySeq x y -> Policy x y
...
instance TheoryNoMonad State Action where
  states :: Action -> [State]
