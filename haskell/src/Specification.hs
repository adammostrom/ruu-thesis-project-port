...
data ActionSpec = Start | Delay | Unit
  deriving (Show, Eq, Enum, Ord)

data StateSpec = DHU | DHC | DLU | DLC | SHU | SHC | SLU | SLC
  deriving (Show, Eq, Enum, Ord)
...
instance Theory ActionSpec StateSpec where 
  ...
  next :: Int -> State StateSpec -> Action ActionSpec -> Prob (State StateSpec)
...
{- 
instance BestExt StateSpec where
  bestExt :: Int -> PolicySeq (State StateSpec) -> Policy (State StateSpec)
    ...
    where
      ...
      bestAction state =
        if state `elem` states
          then
            let actionValues = [(action, val t (Map.singleton (State state) (Action action) : ps_tail) (State state)) | action <- actions]
                best = maximumBy (comparing snd) actionValues
            in (State state, Action (fst best))
          else (State state, Action Start) -}
          
best :: Int -> Int -> State StateSpec -> (ActionSpec, Val)
best t n x
  | n == 0 = error "Horizon must be greater than zero!"
  | otherwise =
      let ps = Theory.bi (t + 1) (n - 1)
          p = Theory.bestExt t ps
          b = fromMaybe Unit (Map.lookup x p)
          vb = val t (p : ps) x
      in (b, vb)

mMeas :: Int -> Int -> State StateSpec -> Double
mMeas t n x
  | x `elem` [SHU, SHC, SLU, SLC] = 0
  | otherwise =
      let ps = Theory.bi t n
          ps' =
            if Map.lookup x (head ps) == Just (Action Start)
              then (Map.insert x (Action Delay) (head ps)) : tail ps
              else (Map.insert x (Action Start) (head ps)) : tail ps
          bestVal = val t ps x
          worstVal = val t ps' x
