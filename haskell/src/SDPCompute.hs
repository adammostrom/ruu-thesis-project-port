...
val _ _ [] _ = 0
val _ t _ _ | t < 0 = error "Time cannot be negative"
val sdp t (p : ps) x =
...
stepValMap sdp (p, t) vNext = Map.fromList [(s, eval s) | s <- states sdp t]
  where
...
optExt cmp sdp t ps = Map.fromList $ map optAction (states sdp t)
  where
    optAction state = (state, getOptAction state)
    getOptAction state =
      let actionsForState = actions sdp t state
          actionValues =
            [ (action, val sdp t (Map.singleton state action : ps) state)
