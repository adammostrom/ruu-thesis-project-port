...
This test file aims to test and assure the properties of the 3 functions: actions, nextFunc, reward
...
def test_actions_return_list(t, state, sdp_instance):  # `t` is provided by Hypothesis
...
@given(st.integers(min_value=0), st.sampled_from(sdp_instance.states), st.sampled_from(list(Action)))
def test_nextFunc_return_dict(t, x, y):
    next = sdp_instance.nextFunc(t, x, y)
    assert(isinstance(next, dict))
    assert all(isinstance(k, State) for k in next.keys()), f"Keys are not all State instances: {next.keys()}"
    assert all(isinstance(v, float) for v in next.values()), f"Values are not all float: {next.values()}"
...
@given(st.integers(min_value=0), st.sampled_from(sdp_instance.states), st.sampled_from(sdp_instance.states), st.sampled_from(sdp_instance.states))
...
def test_bi_recursive_structure(t, n):
    result = sdp_instance.bi(t, n)
    for i in range(1, n):
        assert result[i - 1] != result[i]  # Ensure different policies between steps """
...
def test_bi_monotonicity_consistency(t, n):
    ...
    for i in range(1, n):
        ...
        policies_consistent = any(prev_policy.get(state) == curr_policy.get(state) for state in prev_policy)
        
        assert policies_consistent, f"Policies at step {i} and {i-1} do not show logical evolution."
...
def test_best_output_format():
    ...
    assert re.match(r"Horizon, best, value : \d+, .*?, .*?", result), f"Unexpected output format: {result}"
 """
...
def test_mMeas_monotonicity():
    ...
    for n in range(6, 0, -1):
        ...
        assert curr_value <= prev_value or math.isclose(curr_value, prev_value, abs_tol=1e-10), \
